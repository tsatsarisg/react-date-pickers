...
describe('MonthGrid', () => {
  ...
  describe('Month Override', () => {
    it('displays override month instead of context month', () => {
      render(
        <CalendarProvider defaultValue={createDate(2026, 6, 15)}>
          <MonthGrid month={createDate(2026, 3, 1)} />
        </CalendarProvider>
      );

      expect(screen.getByTestId('day_2026-03-01')).toBeInTheDocument();
      expect(screen.getByTestId('day_2026-03-31')).toBeInTheDocument();
    });
  });
  ...
  describe('Grid Structure', () => {
    ...
    it('uses CSS grid for layout', () => {
      ...
      expect(weekdayRow.style.display).toBe('grid');
      expect(weekdayRow.style.gridTemplateColumns).toBe('repeat(7, 1fr)');
    });
  });
